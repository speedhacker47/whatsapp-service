                ...
                <x-card class="rounded-lg">
                    <x-slot:header>
                        ...
                        <x-settings-description>
                            {{ t('configure_the_unique_feature_for_your_site') }}
                        </x-settings-description>
                    </x-slot:header>
                    <x-slot:content>
                        <!-- Unique Feature Title -->
                        <div class="pt-4">
                            <x-label for="uni_feature_title" :value="t('unique_feature_title')" class="mb-2" />
                            <x-input id="uni_feature_title" type="text" class="mt-1 block w-full"
                                wire:model="uni_feature_title" autocomplete="off" />
                            <x-input-error for="uni_feature_title" class="mt-2" />
                        </div>

                        <!-- Unique Feature Subtitle -->
                        <div class="pt-4">
                            <x-label for="uni_feature_sub_title" :value="t('unique_feature_subtitle')" class="mb-2" />
                            <x-input id="uni_feature_sub_title" type="text" class="mt-1 block w-full"
                                wire:model="uni_feature_sub_title" autocomplete="off" />
                            <x-input-error for="uni_feature_sub_title" class="mt-2" />
                        </div>

                        <!-- Unique Feature Description -->
                        <div class="pt-4">
                            <x-label for="uni_feature_description" :value="t('unique_feature_description')"
                                class="mb-2" />
                            <x-textarea id="uni_feature_description" rows="4" class="mt-1 block w-full"
                                wire:model="uni_feature_description"></x-textarea>
                            <x-input-error for="uni_feature_description" class="mt-2" />
                        </div>

                        <!-- Dynamic List -->
                        <div class="pt-4">
                            <x-label :value="t('unique_feature_list')" class="mb-2" />
                            <div class="space-y-2">
                                @foreach ($uni_feature_list as $index => $listItem)
                                <div class="flex items-center gap-4">
                                    <x-input type="text" class="block w-full" wire:model="uni_feature_list.{{ $index }}"
                                        autocomplete="off" />
                            ...
                            <x-input-error for="uni_feature_list" class="mt-2" />
                        </div>

                        <!-- Unique Feature Image -->
                        <div class="w-full pt-4 upload-section" x-data="fileUploader('uni_feature_image')">
                            <x-label for="uni_feature_image" :value="t('unique_feature_image')" class="mb-2" />
                            ...
                            <div class="relative p-6 border-2 border-dashed rounded-lg cursor-pointer hover:border-info-500 transition duration-300"
                                ...
                                <template x-if="preview">
                                    <div class="relative inline-block">
                                        ...
                                        <button type="button"
                                            ...
                                            x-on:click.stop="confirmDelete($event, () => { clearImage(); $wire.set('uni_feature_image', ''); })">
                                ...
                                <template x-if="!preview && '{{ $featureSettings['theme.uni_feature_image'] }}'">
                                    <div class="relative inline-block">
                                        <img src="{{ asset('storage/' . $featureSettings['theme.uni_feature_image']) }}"
                                            alt="Saved Image Preview"
                                            class="h-24 w-48 object-contain rounded-lg shadow-md" />
                                        <button type="button"
                                            class="absolute -top-4 -right-4 bg-danger-500 text-white rounded-full shadow-lg hover:bg-danger-600"
                                            x-on:click.stop="confirmDelete($event, () => { clearImage(); $wire.removeUniFeatureimage(); setTimeout(() => { location.reload(); }, 1500);})">
                                            <x-heroicon-s-x-circle class="h-5 w-5" />
                                        </button>
                                    </div>
                                </template>
                                ...
                                <input x-ref="imagePathInput" type="file" class="hidden" accept=".png,.jpg,.jpeg"
                                    x-on:change="handleFileChange" wire:model="uni_feature_image"
                            ...
                            <!-- Error Message -->
                            @if ($errors->any())
                            <x-input-error for="uni_feature_image" class="mt-2" />
                            @else
